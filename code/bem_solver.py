...
# ---------------------------------------------------------------------------
# Harmonic mode on the cut torus
# ---------------------------------------------------------------------------
#
# The sheath after the meridional slit is topologically a cylinder.  Its
# first cohomology is one-dimensional, generated by a harmonic 1-form
# representing "uniform flow along the long axis."  On our sheath the long
# axis is the toroidal (phi) direction; the harmonic vector field is
# K_har = (c/s) * phi_hat, axisymmetric on each segment.  This is the
# *signal mode*: it carries net toroidal current that the single-valued ψ
# representation cannot.  Without it the slit-driven BEM only finds the
# linked/poloidal mode (V_eff = 0 by phi-reflection symmetry).
#
# The normalization c is chosen so that the total toroidal current
# crossing any meridional contour at fixed phi is unity.

def harmonic_K_at_elements(mesh: "Mesh") -> Tuple[np.ndarray, float]:
    """Return (K_har Cartesian (n_el, 3), c) for the unit-toroidal-current
    harmonic mode."""
    alpha, beta = mesh.alpha, mesh.beta
    perim = alpha + alpha / (1.0 + beta) + 2.0 * np.log(1.0 + beta)
    c = 1.0 / perim
    K_phi = c / mesh.elem_s              # (n_el,)
    K_x = -K_phi * np.sin(mesh.elem_phi)
    K_y = +K_phi * np.cos(mesh.elem_phi)
    K_z = np.zeros_like(K_phi)
    return np.stack([K_x, K_y, K_z], axis=1), c


def self_single_integral(a: float, b: float) -> float:
    """int_panel dS / r  at the centroid of a rectangular panel of sides
    (a, b).  Closed form:  2 a ln((r+b)/a) + 2 b ln((r+a)/b)  with
    r = sqrt(a^2 + b^2).  Reduces to 4a ln(1+sqrt(2)) for a square."""
    if a <= 0.0 or b <= 0.0:
        return 0.0
    r = np.sqrt(a * a + b * b)
    return 2.0 * a * np.log((r + b) / a) + 2.0 * b * np.log((r + a) / b)


def A_from_K_at_centroids(mesh: "Mesh",
                          K_e: np.ndarray,
                          G: np.ndarray) -> np.ndarray:
    """Vector potential A(c_e) from a piecewise-constant surface current
    K_e (n_el, 3) on the sheath, evaluated at each element centroid c_e.

    Off-diagonal kernel uses centroid quadrature (same as G);
    self contributions use the closed-form single-integral self term.
    Returns A_at_centroids of shape (n_el, 3)."""
    n_el = mesh.elem_xyz.shape[0]
    diag_old = np.diag(G).copy()
    diag_new = np.array([
        mesh.elem_area[e]
        * self_single_integral(mesh.elem_s[e] * mesh.elem_dchi[e],
                                mesh.elem_dt[e])
        for e in range(n_el)
    ])
    G_for_A = G.copy()
    np.fill_diagonal(G_for_A, diag_new)
    A = (G_for_A @ K_e) / (4.0 * np.pi * mesh.elem_area[:, None])
    return A


def assemble_psi_linear_system_har(mesh: "Mesh",
                                    K_har: np.ndarray) -> Tuple:
    """Variational assembly for the sheath-only problem with an external
    axisymmetric harmonic-mode current K_har on the sheath itself.
    Dirichlet BC psi = 0 on both slit edges (the toroidal current is in
    K_har, not in a psi jump).  Returns (A_ii, rhs, psi_bnd_full, A_full,
    c_full, G)."""
    from scipy.sparse import csr_matrix
    n_el = mesh.elem_xyz.shape[0]
    n_n = mesh.n_nodes
    ...
    T_eK = (mesh.grad_chi_coef[:, :, None] * mesh.K_chi_coef_cart[:, None, :]
            + mesh.grad_t_coef[:, :, None] * mesh.K_t_coef_cart[:, None, :])
    corners = mesh.elem_node_idx

    G = assemble_mutual_matrix(mesh)

    # A_full (psi self-energy) -- identical to the wire path
    A_full = np.zeros((n_n, n_n), dtype=np.float64)
    e_idx = np.repeat(np.arange(n_el), 4)
    col_idx = corners.flatten()
    for j in range(3):
        ...
        if not isinstance(GSj, np.ndarray):
            GSj = np.asarray(GSj.todense())
        A_full += np.asarray(S_j.T @ GSj)
    A_full = A_full / (4.0 * np.pi)

    # c_full from K_har coupling
    A_har_cen = A_from_K_at_centroids(mesh, K_har, G)
    v = A_har_cen * mesh.elem_area[:, None]
    Tdotv = np.einsum("ekc,ec->ek", T_eK, v)
    c_full = np.zeros(n_n)
    np.add.at(c_full, corners.flatten(), Tdotv.flatten())

    # Homogeneous Dirichlet on both slit edges (no Delta psi).
    psi_bnd_full = np.zeros(n_n)
    int_idx = mesh.interior_node_idx
    bnd_idx = np.setdiff1d(np.arange(n_n), int_idx)
    A_ii = A_full[np.ix_(int_idx, int_idx)]
    A_ib = A_full[np.ix_(int_idx, bnd_idx)]
    rhs = -A_ib @ psi_bnd_full[bnd_idx] - c_full[int_idx]
    return A_ii, rhs, psi_bnd_full, A_full, c_full, G


def solve_psi_har(mesh: "Mesh", K_har: np.ndarray, verbose: bool = False):
    t0 = time.time()
    A_ii, rhs, psi_bnd_full, A_full, c_full, G = assemble_psi_linear_system_har(
        mesh, K_har)
    t1 = time.time()
    psi_int = la_solve(A_ii, rhs, assume_a="sym")
    t2 = time.time()
    if verbose:
        print(f"Assembly {t1-t0:.1f}s, solve {t2-t1:.1f}s,"
              f" n_int {len(rhs)}", flush=True)
    psi = psi_bnd_full.copy()
    psi[mesh.interior_node_idx] = psi_int
    return psi, dict(A_full=A_full, c_full=c_full, G=G,
                     assemble_time=t1 - t0, solve_time=t2 - t1)


def compute_Lp_har(psi: np.ndarray, mesh: "Mesh", K_har: np.ndarray,
                   info: dict) -> float:
    """L_p = psi^T A psi + 2 psi^T c + L_har_self,
    with L_har_self = (1/(4 pi)) sum_j K_har_j^T G K_har_j."""
    A_full = info["A_full"]
    c_full = info["c_full"]
    G = info["G"]
    L_psi = psi @ (A_full @ psi)
    L_cross = 2.0 * (psi @ c_full)
    L_har = 0.0
    for j in range(3):
        L_har += K_har[:, j] @ (G @ K_har[:, j])
    L_har /= (4.0 * np.pi)
    return L_psi + L_cross + L_har


def compute_Veff_har(psi: np.ndarray, mesh: "Mesh", K_har: np.ndarray,
                     n_s: int = 20, n_phi: int = 50, n_z: int = 20) -> float:
    """V_eff with total K = K_psi + K_har."""
    alpha, beta = mesh.alpha, mesh.beta
    s_nodes, s_w = np.polynomial.legendre.leggauss(n_s)
    z_nodes, z_w = np.polynomial.legendre.leggauss(n_z)
    phi_nodes, phi_w = np.polynomial.legendre.leggauss(n_phi)
    ...
    z_vals = 0.5 * alpha * (z_nodes + 1)
    phi_vals = np.pi * (phi_nodes + 1)
    K_psi = K_from_psi(psi, mesh)
    K_total = K_psi + K_har
    centroids = mesh.elem_xyz
    weights_K = mesh.elem_area / (4 * np.pi)
    total = 0.0
    for phi_v, w_phi in zip(phi_vals, phi_w):
        ss, zz = np.meshgrid(s_vals, z_vals, indexing="ij")
        ws, wz = np.meshgrid(s_w, z_w, indexing="ij")
        xs = ss * np.cos(phi_v); ys = ss * np.sin(phi_v); zs = zz
        pts = np.stack([xs.flatten(), ys.flatten(), zs.flatten()], axis=1)
        diff = pts[:, None, :] - centroids[None, :, :]
        dist = np.sqrt(np.einsum("ijk,ijk->ij", diff, diff))
        Ax = (K_total[None, :, 0] * weights_K[None, :] / dist).sum(axis=1)
        Ay = (K_total[None, :, 1] * weights_K[None, :] / dist).sum(axis=1)
        A_phi = -Ax * np.sin(phi_v) + Ay * np.cos(phi_v)
        local = (A_phi.reshape(s_vals.size, z_vals.size) * ws * wz).sum()
        total += w_phi * local
    total *= (0.5 * beta) * (0.5 * alpha) * np.pi
    return total


def run_one_har(alpha: float,
                beta: float,
                w: float = 0.03,
                n_t_per_seg: int = 15,
                chi_nodes_override: Optional[np.ndarray] = None,
                verbose: bool = False) -> Tuple[RunResult, dict]:
    """Sheath-only run with explicit toroidal harmonic mode.
    Δψ=0 across slit; toroidal current carried by K_har."""
    t0 = time.time()
    mesh = build_mesh(alpha, beta, w=w, n_t_per_seg=n_t_per_seg,
                      chi_nodes_override=chi_nodes_override)
    K_har, c_har = harmonic_K_at_elements(mesh)
    psi, info = solve_psi_har(mesh, K_har, verbose=verbose)
    L_p = compute_Lp_har(psi, mesh, K_har, info)
    V_eff = compute_Veff_har(psi, mesh, K_har)
    t1 = time.time()
    cond = np.linalg.cond(
        info["A_full"][np.ix_(mesh.interior_node_idx,
    ...
    return RunResult(
        ...
        matrix_cond=cond,
        wall_time_s=t1 - t0,
    ), {"psi": psi, "mesh": mesh, "info": info, "K_har": K_har,
         "c_har": c_har}
